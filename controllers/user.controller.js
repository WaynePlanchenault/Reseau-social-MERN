...
module.exports.userInfo = (req, res) => {
  ...
  if (!ObjectID.isValid(req.params.id))
    // test si l'id est connu dans la bdd
...
module.exports.follow = async (req, res) => {
  ...
  try {
    ...
    await UserModel.findByIdAndUpdate(
      ...
      { $addToSet: { following: req.body.idToFollow } }, // ID de la personne qui est suivie
...
module.exports.unfollow = async (req, res) => {
  ...
  try {
    ...
    await UserModel.findByIdAndUpdate(
      req.params.id, //ID de la personne qui veut suivre
      { $pull: { following: req.body.idToUnfollow } }, // ID de la personne qui est suivie
      { new: true, upsert: true },
      (err, docs) => {
        if (!err) res.status(201).json(docs);
        else return res.status(400).json(err);
